...
def get_bpp_public_key_from_header(auth_header):
    ...
    response, status_code = lookup_call('https://preprod.registry.ondc.org/ondc/lookup', payload=payload)
    if status_code == 200:
...
@app.route('/bgSignVerify', methods=['POST'])
def verify_authorisation_header(auth_header=AuthHeader, request_body_str=request_body_raw_text,
    ...
    print("Final AuthHeader Passing:  ", auth_header)
    print('\n')
